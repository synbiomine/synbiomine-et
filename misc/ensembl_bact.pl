...
BEGIN {
#    my $base = ( $0 =~ m:(.*)/.*: )[0];
    my $base = "/home/ml590/src/GitHub/intermine/bio/scripts";
...
#my $script_dir = ( $0 =~ m:(.*)/.*: )[0];
my $script_dir = "/home/ml590/src/GitHub/intermine/bio/scripts";
...
my ( $mine_name, $spec_file, $data_destination ) = @ARGV;
...
my @orgInfo = read_file($spec_file);

for my $species ( @orgInfo ) {
  my ($taxon_id, $strain) = split("\t", $species);
  print "Species Info: ", $taxon_id, "\t", $strain, "\n";
  $organisms{$taxon_id} = "$strain";
}

foreach my $taxon_id ( keys %organisms ) {

  my ($strain_name) = $organisms{$taxon_id};
...
# Taken out to test bacteria look-up
#     my $host =
...
## ML ### my $lookup = Bio::EnsEMBL::LookUp->new();
## ML ### my ($dba) = @{$lookup->get_by_name_exact('bacillus_subtilis_subsp_subtilis_str_168')};
#my ($dba) = @{$lookup->get_by_name_exact('escherichia_coli_str_k_12_substr_mg1655')};

my ($dba) = @{$lookup->get_by_name_exact("$strain_name")};
...
### Slice ###    my $slice_adaptor = $dbCore->get_sliceAdaptor();
...
## ML ##              if ( $taxon_id == 9606 );    # add cross references when run for humans
              if ( $taxon_id );    # add cross reference for all
            writeout( $gene_item );
            writeout( $location );
            writeout( $GeneSOitem );
            ############################################
